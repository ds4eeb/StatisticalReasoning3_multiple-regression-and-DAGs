...
### Q2.1 Identify the "confounds" in the fox DAG
...
Now, let's run a model predicting average food as a function of area:
...
We see a fairly strong effect of area on the average amount of food; because we standardized each variable by standard deviations, our units are now in "standard deviations". (*Note: we can back-transform these value to translate this back to the normal units! We won't do that here, as we'll get a lot more practice with that when we get to generalized linear models, but just know that if you are annoyed by the unitless values, there's a way out!*)
...
Now infer the causal impact of adding food to a territory. Run a model with `weight` as a function of `avgfood`. Do you think more food will make foxes heavier? Interpret your results and whether or not the amount of food increases fox weight according to this model. Are your results expected or unexpected?
...
Think back to the confounds in the DAGs we've looked at. Is there a variable that we should condition upon that may be affecting our results? If so, what variable?
...
### Q2.3: What's your hypothesis about how group size affects fox weight?
...
How does this change your interpretation from the univariate regressions of each variable separately?

### Q2.4c

Provide a small discussion (2-4 sentences) of why you think these results turned out the way they did in the context of the ecological system of fox territories. Include why you think that the univariate regressions may have come out as un-informative.
