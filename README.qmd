...
*Answer*  
*Yes; we should condition upon `groupsize` because of the path from avgfood -> groupsize -> weight, in addition to avgfood -> weight*
...
In the previous model, we saw basically no effect of `avgfood` on `weight`, but we have an extra path that we need to account for, since `avgfood` also flows to `weight` through `groupsize`.

First, let's look at the separate effect of `groupsize` on `weight` in a univariate regression, just like we did with `avgfood`:
...
Interpret the output of this `weight ~ avgfood + groupsize` model.  What are the effects of `avgfood` and `groupsize` now that you have accounted for both variables? Please discuss in a sentence or three.
